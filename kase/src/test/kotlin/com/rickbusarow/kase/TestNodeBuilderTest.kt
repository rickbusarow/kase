...
class TestNodeBuilderTest {
  ...
  fun `root node builder creates containers using testFactory DSL`() {
    val dynamicNodes = listOf("Container1", "Container2")
      .asContainers { Stream.empty() }
  ...
  fun `container builder adds nested containers to a container using testFactory DSL`() {
    val dynamicContainer = listOf("Container1").asContainers {
      listOf("Container2", "Container3").asContainers { Stream.empty() }
    }
