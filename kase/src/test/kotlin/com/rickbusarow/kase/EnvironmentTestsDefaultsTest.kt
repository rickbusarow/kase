...
internal class EnvironmentTestsDefaultsTest : EnvironmentTests<Dog, DogTestEnvironment, DogTestEnvironment.Factory> {

  override val testEnvironmentFactory = DogTestEnvironment.Factory()
  ...
  fun `multi-layer testFactory`() = testFactory(testFactoryKases) { kase ->
    ...
    for ((node, key) in nodes.zip(allKeys)) {
      ...
      val relativeUriPath = Path(node.testSourceUri.get().path).relativeTo(userDir.toPath())
      relativeUriPath shouldBe Path(
        "src/test/kotlin",
        "com/rickbusarow/kase",
        thisStackTrace.fileName!!
      )
    }
  }
